...
static cmark_node *match(cmark_syntax_extension *self,
    ...
    if ((left_flanking || right_flanking) && delims == 1) {
        cmark_inline_parser_push_delimiter(inline_parser,
                                           character,
                                           left_flanking,
                                           right_flanking,
                                           res);
...
static delimiter *insert(cmark_syntax_extension *self,
    ...
    emoji = opener->inl_text;
    
    if (opener->inl_text->as.literal.len != closer->inl_text->as.literal.len)
        goto done;
    
    if (!cmark_node_set_type(emoji, CMARK_NODE_EMOJI))
        goto done;
    
    cmark_node_set_syntax_extension(emoji, self);
    ...
    while (tmp) {
        ...
        cmark_node_append_child(emoji, tmp);
        tmp = next;
    }
    
    emoji->end_column = closer->inl_text->start_column + closer->inl_text->as.literal.len - 1;
...
static int can_contain(cmark_syntax_extension *extension,
                       cmark_node *node,
...
static void latex_render(cmark_syntax_extension *extension,
    ...
    if (entering) {
        renderer->out(renderer, node, "\\UTF{", false, LITERAL);
    } else {
        renderer->out(renderer, node, "}", false, LITERAL);
    }
}
...
static cmark_node *postprocess(cmark_syntax_extension *ext, cmark_parser *parser, cmark_node *root) {
    ...
    while ((ev = cmark_iter_next(iter)) != CMARK_EVENT_DONE) {
        ...
        if (in_emoji) {
            if (ev == CMARK_EVENT_EXIT && node->type == CMARK_NODE_EMOJI) {
                in_emoji = false;
            }
        ...
        if (ev == CMARK_EVENT_ENTER && node->type == CMARK_NODE_EMOJI) {
            in_emoji = true;
            continue;
        }
    }
